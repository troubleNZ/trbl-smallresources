...
RegisterNetEvent("misc:client:UseTent")
AddEventHandler("misc:client:UseTent", function()
--RegisterNetEvent("misc:client:UseTent", function()
    --EquipParachuteAnim()
    QBCore.Functions.Progressbar("use_tent", "Placing Tent..", 5000, false, true, {
        ...
        local sourcePed = GetPlayerPed(ped)
        ...
        local prop = CreateObject(tent, coords.x, coords.y+2, coords.z-1.95, true, false, true)
        SetEntityHeading(prop, GetEntityHeading(ped))
        PlaceObjectOnGroundProperly(prop)
        prevtent = prop
        local tentheading = GetEntityHeading(ped)
        
        exports["qb-target"]:AddBoxZone("tentlocation",vector3(coords.x, coords.y, coords.z), 0.6, 0.6, {
            name="tentlocation",
            heading=tentheading,
            debugPoly=false,
            minZ=z-2,
...
RegisterNetEvent('camping:client:deltent', function(source, args, rawCommand)
--RegisterCommand('deltent', function(source, args, rawCommand)
    if prevtent == 0 then
        print('^8Error: ^0no previous tent spawned, or your previous tent has already been deleted.')
    ...
    TriggerServerEvent("camping:server:pickuptent")
end, false)
--[[
local prevfire = 0
