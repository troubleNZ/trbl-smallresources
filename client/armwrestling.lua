...
if ArmWrestleConfig.showBlipOnMap then
  for _, info in pairs(ArmWrestleConfig.props) do
    info.blip = AddBlipForCoord(info.x, info.y, info.z)
    SetBlipSprite(info.blip, ArmWrestleConfig.blip.id)
    SetBlipDisplay(info.blip, 4)
    SetBlipScale(info.blip, 0.4)
    SetBlipColour(info.blip, ArmWrestleConfig.blip.colour)
    SetBlipAsShortRange(info.blip, true)
  BeginTextCommandSetBlipName("STRING")
    AddTextComponentString(ArmWrestleConfig.blip.title)
...
Citizen.CreateThread(function()
	while true do
		Wait(5000)
    for i, modelConfig in pairs(ArmWrestleConfig.props) do
...
AddEventHandler('evy_arm:start_cl', function()
  ...
  if place == 1 then
    ...
    while grade >= 0.10 and grade <= 0.90 do
      Wait(0)
      PlayFacialAnim(PlayerPedId(), "electrocuted_1", "facials@gen_male@base")
      alert(text[ArmWrestleConfig.language]['tuto'] .. "~INPUT_PICKUP~")
      SetEntityAnimSpeed(PlayerPedId(), "mini@arm_wrestling", "sweep_a", 0.0)
      SetEntityAnimCurrentTime(PlayerPedId(), "mini@arm_wrestling", "sweep_a", grade)
    ...
    if grade >= 0.90 then
      ...
      notify("~g~" .. text[ArmWrestleConfig.language]['win'])
    elseif grade <= 0.10 then
      PlayAnim(PlayerPedId(), "mini@arm_wrestling", "win_a_ped_b", 2)
      notify("~r~" .. text[ArmWrestleConfig.language]['lose'])
    end
    Wait(4000)
...
AddEventHandler('evy_arm:check_cl', function(args)
  ...
  if args == 'place1' then
    ...
    while not started do
      ...
      alert(text[ArmWrestleConfig.language]['wait'])
  ...
  elseif args == 'noplace' then
    ...
    notify('~r~' ..text[ArmWrestleConfig.language]['full'])
